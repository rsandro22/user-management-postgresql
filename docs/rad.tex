...
U suvremenim informacijskim sustavima upravljanje korisničkim računima predstavlja temeljni sigurnosni i organizacijski problem. Gotovo svaka aplikacija koja obrađuje osjetljive ili poslovno važne podatke mora omogućiti autentikaciju korisnika, kontrolu pristupa resursima te praćenje aktivnosti korisnika u sustavu.

Tema ovog rada je razvoj aplikacije za upravljanje korisničkim računima koja podržava različite vrste korisnika, definiranje uloga i prava pristupa te vođenje zapisa o aktivnostima korisnika (audit log). Aplikacija je implementirana korištenjem objektno-relacijske baze podataka PostgreSQL te web aplikacijskog okvira Flask.

Motivacija za odabir teme proizlazi iz česte potrebe za pouzdanim i proširivim mehanizmom kontrole pristupa u poslovnim i informacijskim sustavima. Umjesto dodjeljivanja prava pojedinim korisnicima, primijenjen je RBAC (Role-Based Access Control) pristup koji omogućuje jednostavnije upravljanje autorizacijom i veću fleksibilnost sustava.
\cite{oraictolic2011AkademskoPismoStrategije}.
